...
async def retrieve_event(id: str) -> api.Event:
    ...
    Piccolo is a lot more terse and pleasant than Peewee in some ways. Any
    exceptions should be dealt with by using `HTTPException`.
    Avoid `try/except/finally` blocks like the plague!
    """
...
async def create_event(
    ...
    > `ID` is auto-generated by Piccolo.
    ...
    query = await (
        data.Event.insert(
            data.Event(**event) # Still need `event["creator"] = value` here
        )
