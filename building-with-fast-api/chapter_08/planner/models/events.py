...
# Null values
# -----------
# > Watch out for missing values we're handling internally within the endpoint.
#
# We use `exclude_none=True` when dumping Pydantic models to dictionaries. This 
# is handy when response body does not need to supply a value, but we must make
# sure a `null` value (API layer) gets a value (Data layer) if required.
#
#
...
class Event(BaseModel):
    ...
    > ⚠️ We're using the same model for both request and response here!
    > This means the response hides the `Event.creator` ID.
    
    The `ID` and `Creator` fields are automatically generated by Piccolo. You
    don't have to supply it within the request body.
    """
    title: str
    image: str
    description: str
